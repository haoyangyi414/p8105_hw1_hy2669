...
as.numeric(df$vec_fac)# all values return NA because every element in character and factor vectors does not have a corresponding number,
#while FALSE represent 0 and TRUE represent 1 in logical vectors. It also explain why I can not take the mean of characters and factors.
```
...
head(penguins) # Species,island and sex are factor variables.Bill_length_mm,bill_depth_mm,flipper_length_mm,body_mass_g and year are numeric variables.
nrow(penguins)
